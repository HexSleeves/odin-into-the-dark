...
Engine_Render_Backend :: struct {
	...
	draw_texture_region: proc(ctx: rawptr, texture: Engine_Texture, source, dest: Engine_Rect, origin: Engine_Vec2, rotation: f32, tint: Engine_Color),
}
...
engine_render_draw_rectangle :: proc(engine: ^Engine, x, y, width, height: i32, color: Engine_Color) {
	render := engine_render_backend(engine)
	render.draw_rectangle(render.ctx, x, y, width, height, color)
}

engine_render_draw_rectangle_lines :: proc(engine: ^Engine, x, y, width, height: i32, color: Engine_Color) {
	render := engine_render_backend(engine)
	render.draw_rectangle_lines(render.ctx, x, y, width, height, color)
}

engine_render_draw_text :: proc(engine: ^Engine, text: cstring, x, y, size: i32, color: Engine_Color) {
	render := engine_render_backend(engine)
...
@(private = "file")
raylib_render_draw_rectangle_lines :: proc(ctx: rawptr, x, y, width, height: i32, color: Engine_Color) {
...
@(private = "file")
nil_render_draw_rectangle_lines :: proc(ctx: rawptr, x, y, width, height: i32, color: Engine_Color) {}
