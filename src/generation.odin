...
generate_map :: proc(game: ^Game) {
	// (a) Fill all tiles to Wall explicitly
	for i in 0 ..< MAP_WIDTH * MAP_HEIGHT {
		game.tiles[i] = Tile{}
	}

	// (b) Clear rooms if non-empty
	clear(&game.rooms)

	// (c) Attempt to place rooms
	...
	// (d) Sort rooms by x1 for left-to-right ordering
	slice.sort_by(game.rooms[:], proc(a, b: Room) -> bool {
		return a.x1 < b.x1
	})

	// (e) Connect consecutive rooms with L-shaped corridors
	...
	// (f) Place player at center of first room
	if len(game.rooms) > 0 {
		player_pos := room_center(game.rooms[0])
		game.player.pos = player_pos
	}

	// (g) Place Descent tile at center of last room
	...
	// (h) Scatter 1-2 Rubble tiles in each room
	for &room in game.rooms {
		rubble_count := rand.int_max(2) + 1 // 1 or 2
		for _ in 0 ..< rubble_count {
			...
			for _ in 0 ..< 10 {
				...
				// Don't place on player, descent, or existing rubble
	...
	// (i) Diagnostics log
	fmt.printfln(
		"[gen] seed=%v rooms=%v player=(%v,%v) descent=(%v,%v)",
		game.seed,
		len(game.rooms),
		game.player.pos.x,
		game.player.pos.y,
