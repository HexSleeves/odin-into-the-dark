		...
		for x in 1 ..< MAP_WIDTH - 1 {
			...
			for dir in 0 ..< 4 {
				...
				if nx >= 0 && nx < MAP_WIDTH && ny >= 0 && ny < MAP_HEIGHT {
					...
					if t.type == .Floor || t.type == .Rubble || t.type == .Descent ||
					   t.type == .Water || t.type == .Gas_Vent || t.type == .Unstable {
						has_floor = true
						break
					}
				}
			...
			if depth >= 8 {
				...
				if roll < 10      { ore_type = "iron_ore"; ore_color = iron_color }
				else if roll < 35 { ore_type = "copper_ore"; ore_color = copper_color }
				else if roll < 70 { ore_type = "crystal_shard"; ore_color = crystal_color }
				else              { ore_type = "gold_nugget"; ore_color = gold_color }
			} else if depth >= 5 {
				// iron 20, copper 40, crystal 40
				if roll < 20      { ore_type = "iron_ore"; ore_color = iron_color }
				else if roll < 60 { ore_type = "copper_ore"; ore_color = copper_color }
				else              { ore_type = "crystal_shard"; ore_color = crystal_color }
			} else if depth >= 3 {
				// iron 40, copper 60
				if roll < 40 { ore_type = "iron_ore"; ore_color = iron_color }
				else         { ore_type = "copper_ore"; ore_color = copper_color }
			} else {
				// depth 1-2: only iron
				ore_type = "iron_ore"
			...
			game.ore_veins[idx] = Ore_Vein{ ore_type = ore_type, color = ore_color }
		}
	}
