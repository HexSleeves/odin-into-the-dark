...
UI_Manager :: struct {
	state: gcore.UI_State,
}

ui_manager_make :: proc(use_sprites: bool) -> UI_Manager {
	return UI_Manager{state = gcore.UI_State{use_sprites = use_sprites, inspect_slot = -1}}
}

ui_manager_state :: proc(ui: ^UI_Manager) -> ^gcore.UI_State {
	if ui == nil {
		return nil
	}
	return &ui.state
