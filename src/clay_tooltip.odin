...
when USE_CLAY {
	clay_render_tooltip :: proc(engine: ^eng.Engine, game: ^Game) {
		...
		clay.ElementDeclaration {
			layout = {
				...
				padding = clay.Padding{left = u16(TOOLTIP_PAD_X), right = u16(TOOLTIP_PAD_X), top = u16(TOOLTIP_PAD_Y), bottom = u16(TOOLTIP_PAD_Y)},
			},
			...
			clay.TextDynamic(tooltip_text, {textColor = clay_color(TOOLTIP_TEXT_COLOR), fontSize = u16(TOOLTIP_FONT_SIZE), lineHeight = u16(TOOLTIP_FONT_SIZE)})
		}
	}
	...
	clay_render_gameplay_hints :: proc(engine: ^eng.Engine, game: ^Game) {
		...
		if cur != nil && cur.type == .Anvil {
			clay_render_hint_banner("anvil-hint", "[C = Craft]", i32(MAP_VIEW_HEIGHT) - 22, eng.Engine_Color{160, 160, 170, 255})
		}
		if cur != nil && cur.type == .Fountain {
			clay_render_hint_banner("fountain-hint", "[Fountain — restores HP]", i32(MAP_VIEW_HEIGHT) - 22, eng.Engine_Color{80, 180, 220, 255})
		}
	}
