...
clay_render_tooltip :: proc(engine: ^eng.Engine, game: ^gcore.Game) {
	...
	if box_x + box_w > i32(gcore.MAP_VIEW_WIDTH) {box_x = i32(gcore.MAP_VIEW_WIDTH) - box_w}
	if box_x < 0 {box_x = 0}
	if box_y < 0 {box_y = 0}
	if box_y + box_h > i32(gcore.SCREEN_HEIGHT) {box_y = i32(gcore.SCREEN_HEIGHT) - box_h}
...
clay_render_gameplay_hints :: proc(engine: ^eng.Engine, game: ^gcore.Game) {
...
clay_render_hint_banner :: proc(id: string, text: string, y: i32, color: eng.Engine_Color) {
	...
	clay.ElementDeclaration {
		...
		floating = {
			offset = {f32(gcore.MAP_VIEW_WIDTH / 2), f32(y)},
			attachTo = .Parent,
...
tooltip_text_for_mouse :: proc(engine: ^eng.Engine, game: ^gcore.Game) -> string {
	mouse := eng.engine_mouse_position(engine)
	camera := game_engine_camera_manager(engine)
	if int(mouse.x) >= gcore.MAP_VIEW_WIDTH || int(mouse.y) >= gcore.MAP_VIEW_HEIGHT {
		return ""
	}
	zoom := camera_zoom(camera)
	tile_x := (int(f32(mouse.x) / zoom) + game_camera_x(camera)) / gcore.TILE_SIZE
	tile_y := (int(f32(mouse.y) / zoom) + game_camera_y(camera)) / gcore.TILE_SIZE
	if tile_x < 0 || tile_x >= gcore.MAP_WIDTH || tile_y < 0 || tile_y >= gcore.MAP_HEIGHT {
		return ""
	}
	tile := gcore.tile_at(game, tile_x, tile_y)
	if tile == nil || !gcore.tile_visible_at(game, tile_x, tile_y) {
		return ""
	}
	if game.player.pos.x == tile_x && game.player.pos.y == tile_y {
		return fmt.tprintf("You (%d/%d HP)", game.player.hp, game.player.max_hp)
	}
	enemy := gcore.enemy_at(game, tile_x, tile_y)
	if enemy == nil {
		return ""
	}
	return fmt.tprintf("%s (%d/%d HP)", gcore.enemy_display_name(enemy), enemy.hp, enemy.max_hp)
}
