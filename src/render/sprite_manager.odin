...
sprite_manager_tile :: proc(sprites: ^Sprite_Manager, tile_type: gcore.Tile_Type) -> Sprite {
	if sprites == nil || sprites.backend == nil || sprites.backend == &g_sprites {
...
sprite_manager_fallback :: proc(sprites: ^Sprite_Manager) -> Sprite {
	...
	size := sprites.backend.tile_size if sprites.backend.tile_size > 0 else gcore.SPRITE_SIZE
