...
render_handlers_accept_engine_context_for_runtime_surfaces :: proc(t: ^testing.T) {
	top_level_render: proc(engine: ^eng.Engine, game: ^gcore.Game) = render_game
	map_render: proc(engine: ^eng.Engine, game: ^gcore.Game) = render_map
	webs_render: proc(engine: ^eng.Engine, game: ^gcore.Game) = render_webs
	player_render: proc(engine: ^eng.Engine, game: ^gcore.Game) = render_player
	enemies_render: proc(engine: ^eng.Engine, game: ^gcore.Game) = render_enemies
	items_render: proc(engine: ^eng.Engine, game: ^gcore.Game) = render_items
	clay_screen_render: proc(engine: ^eng.Engine, game: ^gcore.Game) = clay_render_screen_ui
...
map_world_layers_use_shared_shaken_screen_coordinates :: proc(t: ^testing.T) {
	...
	testing.expect(
		t,
		!strings.contains(map_text, "sx := camera_world_x_to_screen(camera, x * gcore.TILE_SIZE)"),
	)
	testing.expect(
		t,
		!strings.contains(
			map_text,
			"px := camera_world_x_to_screen(camera, game.player.pos.x * gcore.TILE_SIZE)",
		),
	)
	testing.expect(
		t,
		!strings.contains(
			map_text,
			"ex := camera_world_x_to_screen(camera, enemy.pos.x * gcore.TILE_SIZE)",
		),
	)
	testing.expect(
		t,
		!strings.contains(
			items_text,
			"ix := camera_world_x_to_screen(camera, item.pos.x * gcore.TILE_SIZE)",
		),
	)
}
