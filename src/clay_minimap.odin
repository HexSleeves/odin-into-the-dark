...
when USE_CLAY {
	clay_render_minimap :: proc(game: ^Game) {
		...
		clay.ElementDeclaration {
			layout = {
				sizing = {
					width = clay.SizingFixed(f32(i32(MAP_WIDTH) * MINIMAP_TILE_SIZE + 4)),
					height = clay.SizingFixed(f32(i32(MAP_HEIGHT) * MINIMAP_TILE_SIZE + 4)),
				},
			...
			floating = {
				offset = {f32(MAP_VIEW_WIDTH - i32(MAP_WIDTH) * MINIMAP_TILE_SIZE - MINIMAP_MARGIN - 2), f32(MINIMAP_MARGIN - 2)},
				attachTo = .Parent,
			...
			for y in 0 ..< MAP_HEIGHT {
				...
				clay.ElementDeclaration {
					layout = {
						sizing = {width = clay.SizingFit(), height = clay.SizingFixed(f32(MINIMAP_TILE_SIZE))},
					...
					for x in 0 ..< MAP_WIDTH {
						if clay.UI(clay.ID("minimap-cell", u32(y * MAP_WIDTH + x)))(
						clay.ElementDeclaration {
							layout = {
								sizing = {
	...
	@(private = "file")
	clay_minimap_cell_color :: proc(game: ^Game, x, y: int) -> eng.Engine_Color {
		if game.player.pos.x == x && game.player.pos.y == y {
			return eng.Engine_Color{255, 255, 0, 255}
		}
		enemy := enemy_at(game, x, y)
