...
player_die :: proc(messages: ^Message_Manager, game: ^Game, cause: string) -> bool {
	...
	logger_debugf(.Enemy, "player died: cause='%s' depth=%v kills=%v", cause, game.depth, game.kills)
...
resolve_attack_player_on_enemy :: proc(
	...
	if enemy.hp <= 0 {
		enemy.alive = false
		logger_debugf(.Enemy, "killed '%s' at (%v,%v) hp_was=%v kills=%v",
			enemy_display_name(enemy), enemy.pos.x, enemy.pos.y, enemy.max_hp, game.kills)
		game.kills += 1
		if enemy.is_boss {
