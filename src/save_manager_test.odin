...
save_manager_save_exists_uses_configured_path :: proc(t: ^testing.T) {
	saves := Save_Manager {
		file_path = "/tmp/itd-save-manager-missing.dat",
	}
...
save_manager_load_accepts_message_manager_context :: proc(t: ^testing.T) {
	save_handler: proc(saves: ^Save_Manager, turns: ^eng.Turn_Manager, game: ^Game) -> bool =
		save_manager_save_game
	load_handler: proc(
			saves: ^Save_Manager,
