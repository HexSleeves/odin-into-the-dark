...
camera_tile_size :: proc(camera: ^eng.Camera_Manager) -> i32 {
	return max(i32(f32(TILE_SIZE) * camera_zoom(camera)), 1)
}
...
visible_tile_bounds :: proc(camera: ^eng.Camera_Manager) -> (x0, y0, x1, y1: int) {
	...
	view_w := int(f32(MAP_VIEW_WIDTH) / zoom)
	view_h := int(f32(MAP_VIEW_HEIGHT) / zoom)
	x0 = max(0, camera_x / TILE_SIZE)
	y0 = max(0, camera_y / TILE_SIZE)
	x1 = min(MAP_WIDTH - 1, (camera_x + view_w) / TILE_SIZE + 1)
	y1 = min(MAP_HEIGHT - 1, (camera_y + view_h) / TILE_SIZE + 1)
	return
}
