...
content_handlers_accept_content_manager_context :: proc(t: ^testing.T) {
	...
	enemy_make_handler: proc(content: ^Content_Manager, id: string, pos: Vec2) -> Enemy = enemy_make
	...
	use_item_handler: proc(content: ^Content_Manager, messages: ^Message_Manager, game: ^Game, slot_index: int) -> bool = use_item
	mine_handler: proc(content: ^Content_Manager, messages: ^Message_Manager, game: ^Game, dx, dy: int) -> bool = mine_wall
	craft_handler: proc(content: ^Content_Manager, messages: ^Message_Manager, game: ^Game, recipe_index: int) = try_craft
