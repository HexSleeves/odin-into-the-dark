...
game_app_registers_current_engine_services :: proc(t: ^testing.T) {
	...
	engine := eng.Engine {
		...
		input = eng.Engine_Input_Backend {
			...
			mouse_position = test_game_app_input_mouse_position,
	...
	was_sprites_loaded := renderer.g_sprites.loaded
	defer renderer.g_sprites.loaded = was_sprites_loaded
	renderer.g_sprites.loaded = true
	...
	// Content manager uses #load'd embedded data — no storage backend.
	testing.expect(t, game_engine_content_manager(&engine).loaded == false)
	...
	testing.expect(t, game_engine_floating_text_manager(&engine) != nil)
	testing.expect(t, game_engine_floating_text_manager(&engine) == &engine.floating_text)
	// 6 app-level (owned copies) + 6 engine-level (non-owned pointers, incl. floating text)
...
@(test)
game_engine_config_audio_backend_matches_audio_build_flag :: proc(t: ^testing.T) {
	config := game_engine_config()
...
test_game_app_input_mouse_position :: proc(ctx: rawptr) -> eng.Engine_Mouse_Position {
	return {}
}
