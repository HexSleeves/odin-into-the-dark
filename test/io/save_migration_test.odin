...
// Legacy v2–v13 read support was intentionally dropped (pre-release; no shipped
// save contract). Any non-v14 version must be rejected cleanly rather than
...
load_save_data_rejects_unsupported_legacy_versions :: proc(t: ^testing.T) {
	for old_version in u32(0) ..= u32(13) {
		// Build a buffer with a valid CRC, then stamp an old version.
		...
		testing.expect(t, floors == nil, "no floors must be returned for an unsupported version")
	}
}
