...
// ─── D1: player light → enemy detection (LIGHT_AFFECTS_DETECTION, default off) ──
...
@(test)
enemy_outside_light_but_in_detection_radius_becomes_aware_when_flag_off :: proc(t: ^testing.T) {
	// Flag off → plain Manhattan: an enemy within detection_radius is detected even
	// though the player emits no light. (When the flag is on this would NOT hold.)
	when !LIGHT_AFFECTS_DETECTION {
		game: Game
		// Distance 5, detection_radius 8, player light 0.
		light_detection_test_setup(&game, Vec2{6, 1}, 8)
...
enemy_in_darkness_falls_back_to_hearing_radius_when_flag_on :: proc(t: ^testing.T) {
	// Flag on → an enemy beyond the hearing radius with no light reaching it is NOT
...
enemy_inside_player_light_becomes_aware_at_full_radius_when_flag_on :: proc(t: ^testing.T) {
	// Flag on → with a wide light radius, the enemy is lit and detected at the full
	// detection_radius.
	when LIGHT_AFFECTS_DETECTION {
		game: Game
		light_detection_test_setup(&game, Vec2{7, 1}, 8)
...
enemy_within_hearing_radius_in_darkness_is_detected_when_flag_on :: proc(t: ^testing.T) {
	// Flag on → an adjacent enemy is heard even in total darkness.
	when LIGHT_AFFECTS_DETECTION {
		game: Game
